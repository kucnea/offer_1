...
export default function Index() {
  ...
  function callCall(){
    window.location.href = `tel:${process.env.NEXT_PUBLIC_REACT_APP_TEL}`;
  }
      ...
      <div key="1" className="w-full h-full flex flex-col min-w-[360px] max-w-[960px]">
        <header className="h-16 flex items-center justify-between px-4 border-b-2 border-gray-300">
          ...
          <div className="h-10 w-32 bg-gray-200 flex items-center justify-center"
                onClick={()=>callCall()}>
            <p> {process.env.NEXT_PUBLIC_REACT_APP_NAME} </p>
          </div>
          <button className="focus:outline-none">
            <svg
              className=" h-6 w-6 text-gray-500 hover:text-gray-600 cursor-pointer"
        ...
        <main className="flex-grow overflow-y-auto">
          <div className="flex flex-col gap-4 p-4">
            ...
            <p className="text-sm text-gray-500">
              문의는
              <strong className="font-medium text-gray-600" onClick={()=>callCall()}>이름</strong> 또는{" "}
              <strong className="font-medium text-gray-600" onClick={()=>callCall()}>번호를</strong>
              클릭해주세요. :)
            </p>
            <div className="flex items-center gap-2">
